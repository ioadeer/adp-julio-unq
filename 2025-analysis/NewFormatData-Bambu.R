# Formateando datos del experimento del Bambu
...
rm(list=ls())
tabla.raw <- read.csv('2025-analysis/data/bambu.csv', 
                      header = TRUE, 
                      sep = ',', 
                      stringsAsFactors = TRUE) %>%
              select(-c("X")) %>%
              rename(perc_distance = Respuesta) %>%
              rename(target_distance = Dist_fis) %>%
              rename(condition = Bloque) %>%
              rename(trial = Trial) %>%
...
results_tbl %>%
  ...
  write_csv("2025-analysis/ResultsData/DResults-bambu.csv")
