...
# esto es de analisis previo, se sacaban estos outliers
...
tabla.raw = tabla.raw %>% rename(room_condition = condicion)   # renombro columnas
# no hay
...
results_tbl <- tibble(aggregate(cbind(percived_distance,signed_bias,unsigned_bias,abs_bias) ~ subject*room_condition*target_distance,
                                data = tabla.raw,
...
results_tbl <- results_tbl %>%
  mutate(
    room_condition = case_when(
      room_condition == "SALA" ~ "Controlled environment",
      room_condition == "INTERNET" ~ "Unknown environment"
    )
  )
